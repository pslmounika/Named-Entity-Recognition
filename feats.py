...
class Feats:
    ...
    def __init__(self):
        ...
        #global LexiconDict
        #self.InvokeLexiconGenerator()

    def InvokdeLexiconGenerator(self):
        """
        LexiconDict=dict()
        f0 = open('C:\Users\Mounika PSL\Desktop\NLP\Homework-6\data\lexicon\lastname.5000', 'r')
        self.LexiconGenerator(f0, "IS_NAME")
        f0.close()
        f0 = open("C:\Users\Mounika PSL\Desktop\NLP\Homework-6\data\lexicon\\firstname.1000", 'r')
        self.LexiconGenerator(f0, "IS_NAME")
        f0.close()
        f0 = open("C:\Users\Mounika PSL\Desktop\NLP\Homework-6\data\lexicon\sports.sports_team", 'r')
        self.LexiconGenerator(f0, "IS_SPORTS")
        f0.close()
        f0 = open("C:\Users\Mounika PSL\Desktop\NLP\Homework-6\data\lexicon\sports.sports_league", 'r')
        self.LexiconGenerator(f0, "IS_SPORTS")
        f0.close()
        f0 = open("C:\Users\Mounika PSL\Desktop\NLP\Homework-6\data\lexicon\location", 'r')
        self.LexiconGenerator(f0, "IS_LOCATION")
        f0.close()
        f0 = open("C:\Users\Mounika PSL\Desktop\NLP\Homework-6\data\lexicon\location.country", 'r')
        self.LexiconGenerator(f0, "IS_LOCATION")
        f0.close()
        """
        ...
        pass
    """
    def LexiconGenerator(self,f,tag):
        for w in f:
            w = w.strip("\n")
            lst = w.split()
            for l in lst:
                if not LexiconDict.has_key(l):
                    LexiconDict[l] = set()
                LexiconDict[l].add(tag)
        pass
